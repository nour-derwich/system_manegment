...
<script type="text/javascript">
 ...
 var oTable1 = $('#buyer_list').dataTable( {
            "aoColumns": [
                { "bSortable": true },
                null,null,null,
                { "bSortable": false }
 ...
 */
 			...
 			$('#scat_id').change(function()
            {
                ...
                var request = $.ajax(
                    {
                        url: "<?php echo base_url('Buyer/get_product_list_dd'); ?>",
                        type: "POST",
                        data: json,
                        dataType: "html",
                        beforeSend : function()
            ...
            $('#product_id').change(function()
            {
               ...
               json['product_id']   = $(this).val();
                var request = $.ajax(
                    {
                        url: "<?php echo base_url('Buyer/get_product_byid'); ?>",
                        type: "POST",
                        data: json,
                        dataType: "json",
                        beforeSend : function()
                        ...
                        {
                            console.log(_return);
                            console.log(_return['_return'][0].product_price);
                           // $('#scat_id').html(_return);
                            //$(".city_get").change();
							/* if(_return['_return'][0].product_price == 0)
							{
										 $.gritter.add({
                                    // (string | mandatory) the heading of the notification
                                    title: 'Product Price Error!',
                                    // (string | mandatory) the text inside the notification
                                    text: 'Please select other product stock not available.',
                                    class_name: 'gritter-success  gritter-light'
                                });
                                $('#product_id').val('');
                                $('#search_cat_id').focus();
							...
							$("#pname").val(_return['_return1'][0]);
							$("#mcost").val(_return['_return'][0].material_cost);
							$("#ocost").val(_return['_return'][0].other_expense);
							/* } */
							//$("#pname").val(_return['_return'][0].product_name);
							//$("#pprice").val(_return['_return'][0].product_price);
			 ...
			 //Show Product
            $('#fdetail').change(function()
            {
                var json             = {};
               json['fdetail_id']   = $(this).val();
                var request = $.ajax(
                    {
                        url: "<?php echo base_url('Seller/get_product_byid'); ?>",
                        type: "POST",
                        data: json,
                        dataType: "json",
                        beforeSend : function()
                        ...
                        success : function(_return)
                        {
                           ...
                           // $('#scat_id').html(_return);
							...
							//$("#pname").val(_return['_return'][0].product_name);
							//$("#pprice").val(_return['_return'][0].product_price);
								$("#fid").val(_return['_return']);
								$("#fname").val(_return['_return1']);
                        },
                        complete: function ()
                        {
                            $('.icon-spinner').addClass('hide');
	...
	//Add Order 
			$("#fqty").blur(function ()
            {
				...
				if(fqty > 0 && fprice > 0)
				{
					final_rem = parseFloat(fqty) * parseFloat(fprice);
					$("#ftprice").val(final_rem);
				}
				
				if($("#square_feet").val() == "" || $("#square_feet").val() == "0")
				{
				...
				{
					var square_feet = parseFloat($("#square_feet").val());
				...
				final_p = ((square_feet * pprice)) + final_rem;
				$('#totalp').val(Math.floor(parseFloat(final_p)));
		...
		$("#pprice").blur(function ()
            {
				...
				{
					var fprice = parseFloat($("#ftprice").val());
				...
				final_p = ((square_feet * pprice)) + fprice;
				$('#totalp').val(Math.floor(parseFloat(final_p)));
			...
			$("#size1").blur(function ()
            {
				...
				if(size1 > 0 && size2 > 0)
				{
					final_rem = parseFloat(size1) * parseFloat(size2) * oqty;
					$("#square_feet").val(final_rem);
...
var size1 = $("#size1").val();
                ...
                final_p = ((final_rem * pprice)) + fprice;
				$('#totalp').val(Math.floor(parseFloat(final_p)));
            });
            ...
            $(".search_seller").click(function()
            {
                ...
                var json = {};
                json['seller_id']  = $('#seller_id').val();
                var request = $.ajax({
                    url: "<?php echo base_url('Seller/seller_search'); ?>",
                    type: "POST",
                    data: json,
                    dataType: "html",
			...
			$("#paid_price_payment").blur(function ()
            {
			   ...
			   else
			   {
				...
				$.gritter.add({
                                    ...
                                    title: 'Paid Price Error!',
                                    // (string | mandatory) the text inside the notification
                                    text: 'Please enter paid price less than & equale to previous price.',
                                    class_name: 'gritter-success  gritter-light'
                                });
            ...
            $('.form-validate-inventory').validate(
                {
                    ...
                    messages: {
                        ...
                        paid_price: " Please enter paid price",
