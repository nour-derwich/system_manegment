...
class Expense extends MX_Controller  
{
    ...
    function __construct()
    {
        ...
        $this->admin_email = $this->Mdl_expense->_get_admin_email();
    }
    ...
    {
        $data['result_set'] = $this->Mdl_expense->_get_expense_list();
    ...
    {
        $data['result_set'] = $this->Mdl_expense->_get_withdraw_list();
    ...
    {
        $data['result_set'] = $this->Mdl_expense->_get_order_list();
    ...
    public function expense_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_expense->_get_expense_list($id);
        }
        $data['content']  = 'Expense/expense_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function withdraw_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_expense->_get_withdraw_list($id);
        }
    ...
    public function sub_category_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_expense->_get_sub_category_list($id);
        }
    ...
    public function expense_submit()
    {
        ...
        if($post['id'] > 0)
        {
			...
			else
			{
				...
				$city_id = $this->session->userdata('city_id');
                    ...
                    $db_command = $this->Mdl_expense->db_command($data_p_relation, $post['id'], 'expense_list');
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Expense/expense_list','refresh');
        ...
        else
        {
           ...
           if($this->session->userdata('user_type') == "1")
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation,null,'expense_list');
                        ...
                        <div class="table-responsive ">
                            <table id="Expense-table" class="table table-striped table-bordered table-hover">
                                ...
                                foreach($result as $res):
                                    ...
                                    $category_name = $this->Mdl_expense->get_relation_pax('category_list','category_name','id',$res->category_id);
                                    if($res->user_type == "seller") {
                                        $seller_name = $this->Mdl_expense->get_relation_pax('seller_list', 'company', 'id', $res->created_by);
                                    }else
                                    {
                                        $seller_name = "--";
                                    }
                                    $subcategory_name = $this->Mdl_expense->get_relation_pax('category_list','category_name','id',$res->subcategory_id);
                                   // $currency_name = $this->Mdl_expense->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                    ?>
                                    <tr>
...
*/
   ...
   //Add New Order
	...
	/* 	$get_order = $this->Mdl_expense->_get_order_code_generator();
    ...
    {	
		...
		exit; */
				...
				if($paid_price < $order_remaining_amount1)
				{
					...
					$db_command = $this->Mdl_expense->db_command($data_p_relation2111,null,'payment_list');
    ...
    public function order_submit()
    {
		...
		//$this->db->trans_start(); // Query will be rolled back
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation1,null,'user_list');
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation1,$post['order_user_id'],'user_list');
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation_order,null,'order_list');
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation_cart,null,'cart_list');
            //$db_command = $this->db->insert('order_list', $data_p_relation_order);
			$last_cart_id = $this->db->insert_id();
				 ...
				 $get_Expense_quantity = $this->Mdl_expense->get_relation_pax('Expense_list','Expense_quantity','id',$pid);
            ...
            $db_command = $this->Mdl_expense->db_command($data_p_relation2,null,'payment_list');
    ...
    {
        $data['result_set'] = $this->Mdl_expense->_this_controller_record_customer_distribution(); 
    ...
    { 
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_expense->_get_single_record_byid($id,'customer');  
        } 
    ...
    public function customer_command()
    {
        ...
        {
            $code_gen = $this->Mdl_expense->_get_customer_code_generator();		
        }
        ...
        $amount = $this->Mdl_expense->check_already_exists_multiple('category_channel',$data_output,$data_amount,2);
        ...
        if($post['login_id'] != null)
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data_login,$post['login_id'],'login');
        ...
        else
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data_login,null,'login');
        ...
        if($post['customer_id'] != null)
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,$post['customer_id'],'customer');
            //$last_login_id = $post['login_id'];
        }
        else
        {	
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,null,'customer');
            //$last_login_id = $this->db->insert_id();
        }
		$this->db->trans_complete(); 
        if($db_command)
    ...
    public function payment_receiving_search()
    {
        ...
        $result_payment =  $this->Mdl_expense->_payment_receiving_search($search_payment);
        //echo "<pre>";
                            ...
                            <table id="payment_table" class="table table-striped table-responsive table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php foreach($result_payment as $fc): 
                                        ...
                                        $category_name = $this->Mdl_expense->get_relation_pax('category_list','title','id',$fc->category_id);
                                        $distribution_name = $this->Mdl_expense->get_relation_pax('distribution_channel','title','id',$fc->dis_channel_id);
                                        $currency_name = $this->Mdl_expense->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                        ?>
                                        <tr>
                                            <td class="center">
    ...
    public function payment_submit()
    {
		...
		foreach($data_post['checkbox_customer'] as $chk)
        {
            ...
            $mobile_no = $this->Mdl_expense->get_relation_pax('customer','mobile_no','id',$chk);
            $shipping = $this->Mdl_expense->get_relation_pax('customer','shipping','id',$chk);
            $amount = $this->Mdl_expense->get_relation_pax('customer','amount','id',$chk);
            $noofcopy = $this->Mdl_expense->get_relation_pax('customer','noofcopy','id',$chk);
            $customer_code = $this->Mdl_expense->get_relation_pax('customer','customer_code','id',$chk);
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,$chk,'customer');
		...
		foreach($data_post['slip_no'] as $slp_key => $slp_val)
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,$slp_key,'customer');
		}	
		...
		foreach($data_post['payment_date'] as $slp_keys => $slp_vals)
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,$slp_keys,'customer');
		}
                            ...
                            <table id="mailing_table" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php foreach($filtered_customer as $fc): 
                                        ...
                                        $Expense_name = $this->Mdl_expense->get_relation_pax('Expense_list','title','id',$fc->Expense_id);
										 $location_name = $this->Mdl_expense->get_relation_pax('location_list','title','id',$fc->location_id);
                                        ?>
                                        <tr>
                                            <td class="center">
                                                <!--<label>
    ...
    public function dispatch_submit()
    {
       ...
       // echo "<pre>";
		...
		foreach($data_post['checkbox_dispatch'] as $chk)
        {
		   ...
		   //Quantity ID
			$quantity_id_get = $this->Mdl_expense->get_relation_pax('inv_distribution','quantity','id',$chk);
			
			
		   //Customer ID
			$customer_id_get = $this->Mdl_expense->get_relation_pax('inv_distribution','customer_id','id',$chk);
			
			//Dispatch Quantity Get
			$dispatch_quantity_get = $this->Mdl_expense->get_relation_pax('customer','dispatch_quantity','id',$customer_id_get);
            ...
            $db_command =  $this->Mdl_expense->db_command($data_inv,$chk,'inv_distribution'); 
            ...
            $db_command =  $this->Mdl_expense->db_command($data_customer,$customer_id_get,'customer'); 
    ...
    public function print_card()
    {
        ...
        if($id != null)
        {
            ...
            else
            {
                ...
                $city_name = $this->Mdl_expense->get_relation_pax('city_list','title','id',$filtered_customer[0]->city_id);
                $location_name = $this->Mdl_expense->get_relation_pax('location_list','title','id',$distribution[0]->location_id);
                //echo "<pre>";
    ...
    { 
        ...
        if(isset($_POST['save_inventory']))
        {
            ...
            $db_command =  $this->Mdl_expense->db_command($data,$post['id'],'inv_receiving');
            if($db_command)
            {
                if($post['id'] != null)
                {
    ...
    public function check_user_exists()
    {
        ...
        $id 	=  $post['id'];
        $cmd_return         =  $this->Mdl_expense->_get_single_record_byusername($username,$id);
    ...
    public function check_email_exists()
    {
        ...
        $email           =  $post['email'];
        $id                 =  $post['id'];
        $cmd_return         =  $this->Mdl_expense->_get_single_record_byemail($email,$id);
    ...
    public function check_phone_exists()
    {
        ...
        $mobile_no           =  $post['mobile_no'];
        $id                 =  $post['id'];
        $cmd_return         =  $this->Mdl_expense->_get_single_record_byphone($mobile_no,$id);
    ...
    public function get_category_channel_list()
    {
        ...
        $data = $this->Mdl_expense->_get_category_channel_list($category_id,$dis_channel_id);



        if($data[0]->rate > 0)
        {
            $currency = $this->Mdl_expense->get_relation_pax('currency_list','code','id',$data[0]->currency_id);
    ...
    public function get_subcategory_byid()
    {
        ...
        $data['result_set'] = $this->Mdl_expense->sub_category_list($search_cat_id);
    ...
    public function get_Expense_byid()
    {
        ...
        $_return['_return'] = $this->Mdl_expense->_get_single_Expense_byCategory($search_cat_id,$scat_id);
        $return             = json_encode($_return);
        echo $return;
    ...
    public function change_category_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_expense->_change_category_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
    ...
    public function change_Expense_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_expense->_change_Expense_status($id,$status);
        if($_return['_return'] == 1)
        {
            //Active Account
            if($status == 0)
            {
                $seller_detail = $this->Mdl_expense->_this_seller_controller_record($seller_id);
    ...
    public function change_discount_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_expense->_change_discount_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
