...
class Mdl_staff extends CI_Model {
	...
	{
		return	$this->db->get_where($type,array('staff_id'=>$type_id))->row()->$field;	
	}
	
	
	 //Order Code Generator
    public function _get_student_code_generator()
    {
		$this->db->select('*');
        $this->db->order_by('staff_id','desc');
        $this->db->limit(1);
        $result = $this->db->get('staff_list');
        if( $result->num_rows() > 0 )
        {
            $last_num = $result->row_array();
            $initel_num = $last_num['staff_code'];
	...
	{
		$query	=	$this->db->get_where('staff_list' , array('class_id' => $class_id));
	...
	{
		$query	=	$this->db->get_where('staff_list' , array('staff_id' => $student_id));
		return $query->result_array();
	}
	/////////TEACHER/////////////
