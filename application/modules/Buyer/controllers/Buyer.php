...
class Buyer extends MX_Controller  
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Mdl_buyer');
        $this->load->library('Pdf');
        $this->admin_email = $this->Mdl_buyer->_get_admin_email();
		$this->system_info = $this->Mdl_buyer->_get_system_info();
		//print_r($data['system_info']);
    }

    public function index()
    {    
       $data['result_set'] = $this->Mdl_buyer->_get_category_list();
        $data['content']  = 'Buyer/category_list';
    ...
    public function buyer_list()
    {
            ...
            $data['result_set'] = $this->Mdl_buyer->_this_controller_record();
            $data['content']    = 'Buyer/buyer_list';
            $this->load->view('Template/template',$data);
    ...
    {
        $data['result_set'] = $this->Mdl_buyer->_get_sub_category_list();
        $data['content']  = 'Buyer/sub_category_list';
        $this->load->view('Template/template',$data);
    }
  ...
  //Order List
    public function order_list()
    {
        ...
        $data['result_set'] = $this->Mdl_buyer->_this_order_record($id);
        $data['content']    = 'Buyer/order_list';
        $this->load->view('Template/template',$data);
    }
    //Order List
    public function view_order_list()
    {
        $data['result_set'] = $this->Mdl_buyer->_get_order_list();
        $data['content']  = 'Buyer/view_order_list';
        $this->load->view('Template/template',$data);
    }
    ...
    {
        $id= $this->uri->segment(3);
        $data['result_set'] = $this->Mdl_buyer->_this_cart_record($id);
        $data['content']    = 'Buyer/cart_list';
        $this->load->view('Template/template',$data);
    }
    ...
    public function view_payment_list()
    {
        ...
        $data['result_set'] = $this->Mdl_buyer->_get_payment_list($user_id,$order_code);
       
        $data['content']    = 'Buyer/payment_list';
        $this->load->view('Template/template',$data);
    }
...
//Slip List
    public function view_order_slip()
    {
       ...
       // $data['content']    = 'Buyer/view_order_slip';
        $this->load->view('Buyer/view_order_slip',$data);
    }
    ...
    public function view_order_slip1()
    {
        ...
        $this->load->view('Buyer/view_order_slip1',$data);
    }
    ...
    public function category_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_buyer->_get_category_list($id);
        }
        $data['content']  = 'Buyer/category_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function sub_category_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_buyer->_get_sub_category_list($id);
        }
        $data['content']  = 'Buyer/sub_category_action';
        $this->load->view('Template/template',$data);
    }

    //Buyer List Action
    public function Buyer_action()
    {
        $id = $this->uri->segment(3);
        $data['get_Buyer'] = $this->Mdl_buyer->_get_single_Buyer($id);
        $data['content']  = 'Buyer/Buyer_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function category_submit()
    {
        ...
        if($post['id'] > 0)
        {
            ...
            if($_FILES['cat_img']['name']!="")
            {
                ...
                $db_command = $this->Mdl_buyer->db_command($data_p_relation,$post['id'],'category_list');
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Buyer/category_list','refresh');
        ...
        else
        {
          ...
          //  exit;
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation,null,'category_list');
            $this->db->trans_complete();
            if($db_command)
            {
                if($post['id'] > 0)
        ...
        if($post['id'] > 0)
        {
            ...
            else
            {
                if($post['image_status'] == 1) {
                    ...
                    $db_command = $this->Mdl_buyer->db_command($data_p_relation1, $post['id'], 'category_list');
                    $this->db->trans_complete();
                }else
                {
                    $this->db->trans_start(); // Query will be rolled back
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Buyer/sub_category_list','refresh');
    ...
    public function Buyer_submit()
    {
        ...
        $Buyer_code = $this->Mdl_buyer->_get_Buyer_code_generator();
       ...
       // exit;
        if($post['id'] > 0)
        {
            ...
            if($_FILES['Buyer_img']['name']!="")
            {
                $targetDir = FCPATH."public_html/frontend/image/Buyer/";
                $fileName ="Buyer_".time().$this->session->userdata('id').'.jpg';
                move_uploaded_file($_FILES['Buyer_img']['tmp_name'], $targetDir.$fileName);
                $cat_image = $fileName;
            }
            else
            {
            ...
            if(isset($_FILES['mBuyer_img']) && count($_FILES['mBuyer_img']['error']) == 1 && $_FILES['mBuyer_img']['error'][0] > 0)
            {
            ...
            else
            {
                $images_arr = array();
                $i = 1;
                foreach ($_FILES['mBuyer_img']['name'] as $key => $val) {
                    //upload and stored images
                    $targetDir = FCPATH . "public_html/frontend/image/Buyer/";
                    $fileName = "subBuyer_" . time() . $i . $this->session->userdata('id') . '.jpg';
                    if (move_uploaded_file($_FILES['mBuyer_img']['tmp_name'][$key], $targetDir . $fileName)) {
                        $images_arr[] = $fileName;
                    }
                    $i++;
                }
            ...
            $data_p_relation =
                ...
                (
                    ...
                    'is_new'                   => $post['Buyer_type'],
                    'Buyer_name'             => $post['Buyer_name'],
                    'Buyer_description'      => $post['Buyer_des'],
                    'Buyer_featured'         => $post['Buyer_fea'],
                    'Buyer_price'            => $post['price'],
                    ...
                    'Buyer_quantity'         => $post['qty'],
                    'Buyer_image'            => $cat_image,
                    'featured_image'           => $featured_image,
                    'user_type'                => $user_type,
                    'is_enable'                => $is_enable
                );
            $db_command = $this->Mdl_buyer->db_command($data_p_relation,$post['id'],'Buyer_list');
            $this->db->trans_complete();
            if($db_command)
            {
                if($post['id'] > 0)
                ...
                redirect(base_url().'Buyer/Buyer_list','refresh');
            }
            else
            {
                $this->session->set_flashdata('Error', 'your data has not been saved');
        ...
        else
        {
           ...
           // exit;
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation,null,'Buyer_list');
            $this->db->trans_complete();


            if($db_command)
            {
                //Send Email Admin Buyer ADD Seller
                if($this->session->userdata('user_type') == 'seller')
                {
                    ...
                    $msg1 = "Dear Admin Seller Buyer Add at Online Shopping";
                    $msg1 .= "<br/><a href='".$get_link1."' >CLICK HERE</a> to login your account and view Buyer detail to active Buyer in website.<br/>Thank you";
                    $this->email->from($from_email1, 'Online Shopping');
                    $this->email->to($to_email1);
                    $this->email->subject('Seller Buyer Add at Online Shopping');
    ...
    public function Buyer_search()
    {
        ...
        $result =  $this->db->get('Buyer_list')->result();
        ?>
                ...
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <div class="table-responsive ">
                            <table id="Buyer-table" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th>Seller</th>
                                    <th>Category</th>
                                ...
                                foreach($result as $res):
                                    ...
                                    $category_name = $this->Mdl_buyer->get_relation_pax('category_list','category_name','id',$res->category_id);
                                    if($res->user_type == "seller") {
                                        $seller_name = $this->Mdl_buyer->get_relation_pax('seller_list', 'company', 'id', $res->created_by);
                                    }else
                                    {
                                        $seller_name = "--";
                                    }
                                    $subcategory_name = $this->Mdl_buyer->get_relation_pax('category_list','category_name','id',$res->subcategory_id);
                                   // $currency_name = $this->Mdl_buyer->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                    ?>
                                    <tr>
                                        <td class="center">
                                            <?php
                                        ...
                                        <td><?php echo $res->Buyer_description;?></td>
                                        <td><?php echo $res->Buyer_featured;?></td>
                                        <td>
                                            <a class="green" target="_blank" href="<?php echo base_url('Buyer/view_image/'.$res->id.''); ?>">
                                                <i class="icon-eye-open bigger-130"></i>
                                            </a>
                                        </td>
                                        <td><?php echo $res->Buyer_price;?></td>
                                        ...
                                        <td><?php echo $res->Buyer_quantity;?></td>

                                        <td>
                                            <label class=" inline">
                                                <input  ref="<?php echo $res->id; ?>" reff="<?php if($res->user_type == 'seller'){ echo $res->created_by; } ?>"  <?php if($res->is_enable == 0){ echo 'value="0" ';}else{echo 'value="1" checked="checked"';}  ?>    type="checkbox" class="ace ace-switch ace-switch-5 <?php if($this->session->userdata('user_type') != 'seller'){ echo "Buyer_status"; } ?>" />
                                                <span class="lbl"></span>
                                            </label>
                                        </td>
                                        ...
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                ...
                                                <a class="green"  href="<?php echo base_url('Buyer/Buyer_action/'.$res->id.''); ?>">
                                            ...
                                            <div class="visible-xs visible-sm hidden-md hidden-lg">
                                                <div class="inline position-relative">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
                                                        ...
                                                        <li>
                                                            <a href="<?php echo base_url('Buyer/Buyer_action/'.$res->id.''); ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
                                                                <span class="green">
            ...
            jQuery(function($)
            {
                ...
                $(".Buyer_status").change(function(){
                    ...
                    if(ref != null)
                    {
                        ...
                        var request = $.ajax({
                            url: "<?php echo base_url('Buyer/change_Buyer_status'); ?>",
                            type: "POST",
                            data: json,
                            dataType: "json",
                           // dataType: "html",
                            ...
                            success : function(_return)
                            {
                                ...
                                if(_return['_return'] > 0)
                                {
                                    ...
                                    $.gritter.add({
                                        ...
                                        text: 'Buyer Status Changed Successfully.',
  ...
  /*              //Discount Status Change
                $(".discount_status").change(function(){
                    ...
                    if(ref != null)
                    {
                        ...
                        json['status']   = status_new;
                        var request = $.ajax({
                            url: "<?php //echo base_url('Buyer/change_discount_status'); ?>",
                            type: "POST",
                            data: json,
                            dataType: "json",
...
*/
                var oTable1 = $('#Buyer-table').dataTable(
                    {
                        "aoColumns":
                            [
                                { "bSortable": true },
                                ...
                                foreach($result as $res):
                                   ...
                                   // $currency_name = $this->Mdl_buyer->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="green btn" target="_blank" href="<?php echo base_url('Buyer/add_order/'.$res->id.''); ?>">
                                               Buy Now
                                            </a>
                                        </td>
...
*/
   ...
   //Add New Order
	...
	/* 	$get_order = $this->Mdl_buyer->_get_order_code_generator();
    ...
    {
        $data['content']  = 'Buyer/add_payment';
        $this->load->view('Template/template',$data);
    }
    ...
    {	
		...
		exit; */
				...
				if($paid_price < $order_remaining_amount1)
				{
					...
					$db_command = $this->Mdl_buyer->db_command($data_p_relation2111,null,'payment_list');
            ...
            if($db_command)
            {
				...
				//$data['content']    = 'Buyer/view_order_slip';
				//$this->load->view('Buyer/order_list',$data);
				redirect(base_url().'Buyer/order_list/','refresh');
            }
            else
            {
                $this->session->set_flashdata('Error', 'your payment has not been saved');
                redirect(base_url().'Buyer/add_payment','refresh');
    ...
    public function order_submit()
    {
		...
		//$this->db->trans_start(); // Query will be rolled back
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation1,null,'user_list');
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation1,$post['order_user_id'],'user_list');
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation_order,null,'order_list');
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation_cart,null,'cart_list');
            //$db_command = $this->db->insert('order_list', $data_p_relation_order);
            ...
            $db_command = $this->Mdl_buyer->db_command($data_p_relation2,null,'payment_list');
			$last_payment_id = $this->db->insert_id();
            ...
            else
            {
                ...
                redirect(base_url().'Buyer/add_order/','refresh');
    ...
    public function view_image()
    {
        ...
        $data['content']  = 'Buyer/Buyer_view_image';
        $this->load->view('Template/template',$data);
    ...
    {
        $data['result_set'] = $this->Mdl_buyer->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Buyer/distribution';
        $this->load->view('Template/template',$data);   
    }
    ...
    { 
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_buyer->_get_single_record_byid($id,'customer');  
        } 

        $data['content']   = 'Buyer/customer_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function customer_command()
    {
        ...
        {
            $code_gen = $this->Mdl_buyer->_get_customer_code_generator();		
        }
        ...
        $amount = $this->Mdl_buyer->check_already_exists_multiple('category_channel',$data_output,$data_amount,2);
        ...
        if($post['login_id'] != null)
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_login,$post['login_id'],'login');
        ...
        else
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_login,null,'login');
        ...
        if($post['customer_id'] != null)
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,$post['customer_id'],'customer');
            //$last_login_id = $post['login_id'];
        }
        else
        {	
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,null,'customer');
            //$last_login_id = $this->db->insert_id();
        }
		$this->db->trans_complete(); 
        if($db_command)
        {
            if($post['customer_id'] != null)
            {
                ...
                $this->session->set_flashdata('update', 'your data successfully Updated');  
                redirect(base_url()."Buyer/customer",'refresh'); 
            }
            else
            {
                ...
                $this->session->set_flashdata('saved', 'your data successfully Saved'); 
                redirect(base_url()."Buyer/success/$get_code/$code_gen/$get_code",'refresh'); 
            }
    ...
    {
        //$data['result_set'] = $this->Mdl_buyer->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Buyer/payment_receiving';
        $this->load->view('Template/template',$data);   
    }
    ...
    public function payment_receiving_search()
    {
        ...
        $result_payment =  $this->Mdl_buyer->_payment_receiving_search($search_payment);
        //echo "<pre>";
                            ...
                            <table id="payment_table" class="table table-striped table-responsive table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php foreach($result_payment as $fc): 
                                        ...
                                        $category_name = $this->Mdl_buyer->get_relation_pax('category_list','title','id',$fc->category_id);
                                        $distribution_name = $this->Mdl_buyer->get_relation_pax('distribution_channel','title','id',$fc->dis_channel_id);
                                        $currency_name = $this->Mdl_buyer->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                        ?>
                                        <tr>
                                            <td class="center">
											...
											<td>
											 <?php 
                                                if($fc->payment_status == 0)
                                                {
                                                    ?>
												<div class="clearfix">	
											 <input type="date" name="payment_date[<?php echo $fc->id; ?>]" id="payment_date" value="<?php //echo $fc->id; ?>" class="date-picker col-xs-3 col-sm-9" placeholder="Payment Date" data-date-Buyerat="yyyy-mm-dd"  />
											 </div>
											 <?php
												}
												else
        ...
        <div class="clearfix Buyer-actions" style="text-align: center;">
    ...
    public function payment_submit()
    {
		...
		foreach($data_post['checkbox_customer'] as $chk)
        {
            ...
            $mobile_no = $this->Mdl_buyer->get_relation_pax('customer','mobile_no','id',$chk);
            $shipping = $this->Mdl_buyer->get_relation_pax('customer','shipping','id',$chk);
            $amount = $this->Mdl_buyer->get_relation_pax('customer','amount','id',$chk);
            $noofcopy = $this->Mdl_buyer->get_relation_pax('customer','noofcopy','id',$chk);
            $customer_code = $this->Mdl_buyer->get_relation_pax('customer','customer_code','id',$chk);
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,$chk,'customer');
		...
		foreach($data_post['slip_no'] as $slp_key => $slp_val)
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,$slp_key,'customer');
		}	
		...
		foreach($data_post['payment_date'] as $slp_keys => $slp_vals)
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,$slp_keys,'customer');
		}
		
		
        $this->session->set_flashdata('update', 'your payment has been received successfully');  
        redirect(base_url()."Buyer/payment_receiving",'refresh'); 
    }
    ...
    public function actionSms($number,$msg)
    {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER,     array('Content-Type: application/x-www-Buyer-urlencoded'));
    ...
    {
        //$data['result_set'] = $this->Mdl_buyer->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Buyer/mailing_list';
        $this->load->view('Template/template',$data);   
    }
    ...
    public function get_customer_mailing_list()
    {
        ...
        $this->db->select('inv_distribution.id as inv_id,inv_distribution.trans_code,inv_distribution.trans_date,customer.*,customer.id,customer.name,customer.email,customer.noofcopy,customer.category_id,inv_distribution.Buyer_id,inv_distribution.location_id');
        if($trans_code > 0)
        {
        ...
        if($Buyer_id > 0)
        {
            $this->db->where('inv_distribution.Buyer_id',$Buyer_id);
        }
        if($location_id > 0)
        {
                            ...
                            <table id="mailing_table" class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php foreach($filtered_customer as $fc): 
                                        ...
                                        $Buyer_name = $this->Mdl_buyer->get_relation_pax('Buyer_list','title','id',$fc->Buyer_id);
										 $location_name = $this->Mdl_buyer->get_relation_pax('location_list','title','id',$fc->location_id);
                                        ?>
                                        <tr>
                                            <td class="center">
                                                <!--<label>
                                            ...
                                            <td><?php echo $fc->noofcopy; ?> </td>
                                            <td>
                                                <a href="<?php echo base_url('Buyer/print_card/'.$fc->inv_id.''); ?>" class="tooltip-error" data-rel="tooltip" title="View" target="_blank" >
                                                    <span class="purple">
    ...
    public function get_customer_dispatch_list()
    {
        ...
        $this->db->select('inv_distribution.id as inv_id,inv_distribution.trans_code,inv_distribution.trans_date,customer.id,customer.*,customer.name,customer.email,customer.noofcopy,customer.category_id,inv_distribution.Buyer_id,inv_distribution.location_id');
        if($trans_code > 0)
        {
            $this->db->where('inv_distribution.trans_code',$trans_code);
        }
    ...
    public function dispatch_submit()
    {
       ...
       // echo "<pre>";
		...
		foreach($data_post['checkbox_dispatch'] as $chk)
        {
		   ...
		   //Quantity ID
			$quantity_id_get = $this->Mdl_buyer->get_relation_pax('inv_distribution','quantity','id',$chk);
			
			
		   //Customer ID
			$customer_id_get = $this->Mdl_buyer->get_relation_pax('inv_distribution','customer_id','id',$chk);
			
			//Dispatch Quantity Get
			$dispatch_quantity_get = $this->Mdl_buyer->get_relation_pax('customer','dispatch_quantity','id',$customer_id_get);
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_inv,$chk,'inv_distribution'); 
            ...
            $db_command =  $this->Mdl_buyer->db_command($data_customer,$customer_id_get,'customer'); 
        ...
        redirect(base_url()."Buyer/dispatch_list",'refresh'); 
    }
    ...
    public function print_card()
    {
        ...
        if($id != null)
        {
            if($id  == 0)
            {
                ...
                redirect(base_url().'Buyer/mailing_list','refresh');   
            }
            else
            {
                $this->db->where('id',$id);
                ...
                $city_name = $this->Mdl_buyer->get_relation_pax('city_list','title','id',$filtered_customer[0]->city_id);
                $location_name = $this->Mdl_buyer->get_relation_pax('location_list','title','id',$distribution[0]->location_id);
                //echo "<pre>";
    ...
    { 
        ...
        if(isset($_POST['save_inventory']))
        {
            ...
            $db_command =  $this->Mdl_buyer->db_command($data,$post['id'],'inv_receiving');
            if($db_command)
            {
                if($post['id'] != null)
                {
                ...
                redirect(base_url().'Buyer/inventory_receiving','refresh');  
            } 
        }
        else 
        {
    ...
    {
        //$data['result_set'] = $this->Mdl_buyer->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Buyer/dispatch';
        $this->load->view('Template/template',$data);   
    }
	
	//Recent SMS List Buyer
	public function resend_sms()
    {
        //$data['result_set'] = $this->Mdl_buyer->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Buyer/resend_sms';
        $this->load->view('Template/template',$data);   
    }
    ...
    public function recent_sms_submit()
    {
        ...
        redirect(base_url()."Buyer/resend_sms",'refresh'); 
    }
    ...
    public function check_user_exists()
    {
        ...
        $id 	=  $post['id'];
        $cmd_return         =  $this->Mdl_buyer->_get_single_record_byusername($username,$id);
    ...
    public function check_email_exists()
    {
        ...
        $email           =  $post['email'];
        $id                 =  $post['id'];
        $cmd_return         =  $this->Mdl_buyer->_get_single_record_byemail($email,$id);
    ...
    public function check_phone_exists()
    {
        ...
        $mobile_no           =  $post['mobile_no'];
        $id                 =  $post['id'];
        $cmd_return         =  $this->Mdl_buyer->_get_single_record_byphone($mobile_no,$id);
    ...
    public function get_category_channel_list()
    {
        ...
        $data = $this->Mdl_buyer->_get_category_channel_list($category_id,$dis_channel_id);



        if($data[0]->rate > 0)
        {
            $currency = $this->Mdl_buyer->get_relation_pax('currency_list','code','id',$data[0]->currency_id);
    ...
    public function get_subcategory_byid()
    {
        ...
        $data['result_set'] = $this->Mdl_buyer->sub_category_list($search_cat_id);
	 ...
	 //Get Product List
    public function get_product_list_dd()
    {
        $search_cat_id         =  $this->input->post('search_cat_id');
        $scat_id         =  $this->input->post('scat_id');
        $data['result_set'] = $this->Mdl_buyer->get_product_list_dd($search_cat_id,$scat_id);
        foreach($data['result_set'] as $key => $value):
            $option .= '<option value = '.$key.' >'.$value.'</option>';
        endforeach;
        echo $option;
    ...
    public function get_product_byid()
    {
		...
		$product_name = $this->Mdl_buyer->get_relation_pax("product_list","product_name","id",$product_id);
		
        $_return['_return'] = $this->Mdl_buyer->_get_single_product_byCategory($product_id);
    ...
    public function change_category_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_buyer->_change_category_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
    ...
    //Change Buyer Status
    public function change_Buyer_status()
    {
        $post               =  $this->input->post();
        ...
        $_return['_return'] =  $this->Mdl_buyer->_change_Buyer_status($id,$status);
        if($_return['_return'] == 1)
        {
            //Active Account
            if($status == 0)
            {
                $seller_detail = $this->Mdl_buyer->_this_seller_controller_record($seller_id);
                $from_email1 = $this->session->userdata('email');
                $to_email1 = $seller_detail[0]->email;
                 //Load email library
                $get_link1 = base_url()."admin";
                $username = $seller_detail[0]->username;
                $msg1 = "Dear ".$$username."<br/>Your Buyer has been Active at Online Shopping";
                $msg1 .= "<br/><a href='".$get_link1."' >CLICK HERE</a> to login your account and view your Buyers.<br/>Thank you";
                $this->email->from($from_email1, 'Online Shopping');
                $this->email->to($to_email1);
                $this->email->subject('Seller Buyer Active at Online Shopping');
                $this->email->message($msg1);
                $this->email->send();
            ...
            else
            {
                ...
                $get_link1 = base_url()."admin";
                $username = $seller_detail[0]->username;
                $msg1 = "Dear ".$$username."<br/>Your Buyer has been De-Active at Online Shopping";
                $msg1 .= "<br/>please contact site admin.<br/>Thank you";
                $this->email->from($from_email1, 'Online Shopping');
                $this->email->to($to_email1);
                $this->email->subject('Seller Buyer DeActive at Online Shopping');
                $this->email->message($msg1);
                $this->email->send();
    ...
    public function change_discount_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_buyer->_change_discount_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
