...
class Bank extends MX_Controller  
{
    ...
    function __construct()
    {
        ...
        $this->admin_email = $this->Mdl_bank->_get_admin_email();
    }

    public function index()
    {    
       $data['result_set'] = $this->Mdl_bank->_get_category_list();
        $data['content']  = 'Bank/category_list';
    ...
    {
        $data['result_set'] = $this->Mdl_bank->_get_bank_list();
    ...
    {
        $data['result_set'] = $this->Mdl_bank->_get_check_list();
        $data['content']  = 'Bank/check_list';
        $this->load->view('Template/template',$data);
    }
	
	 //Buyer Check List
    public function buyer_check_list()
    {
        $data['result_set'] = $this->Mdl_bank->_get_check_list1();
        $data['content']  = 'Bank/buyer_check_list';
        $this->load->view('Template/template',$data);
    }
    ...
    public function bank_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_bank->_get_bank_list($id);
        }
        $data['content']  = 'Bank/bank_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function check_action()
    {
        ...
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_bank->_get_check_list($id);
        }
        $data['content']  = 'Bank/check_action';
        $this->load->view('Template/template',$data);
    }
   
//Check List Action
    public function buyer_check_action()
    {
        $id = $this->uri->segment(3);
        if($id > 0)
        {
            $data['result_set'] = $this->Mdl_bank->_get_check_list1($id);
        }
        $data['content']  = 'Bank/buyer_check_action';
        $this->load->view('Template/template',$data);
    }
   ...
   //bank Submit
    public function bank_submit()
    {
        ...
        if($post['id'] > 0)
        {
                ...
                $db_command = $this->Mdl_bank->db_command($data_p_relation,$post['id'],'bank_list');
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Bank/bank_list','refresh');
        ...
        else
        {
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation,null,'bank_list');
    ...
    public function check_submit()
    {
        ...
        if($post['id'] > 0)
        {
                    ...
                    $data_p_relation1 =
                        ...
                        (
                            ...
                            'check_clear_date' => $post['check_clear_date'],
                            'check_status' => $post['check_status'],
                            'is_enable' => 1
                        );
                    $db_command = $this->Mdl_bank->db_command($data_p_relation1, $post['id'], 'check_list');
                    $this->db->trans_complete();
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Bank/check_list','refresh');

            }
            else
            {
                $this->session->set_flashdata('Error', 'your data has not been saved');
                redirect(base_url().'Bank/sub_category_list','refresh');
        ...
        else
        {
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation,null,'check_list');
            $this->db->trans_complete();
            if($db_command)
            {
                if($post['id'] > 0)
	 ...
	 //Buyer Check Submit
    public function buyer_check_submit()
    {
        ...
        if($post['id'] > 0)
        {
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Bank/buyer_check_list','refresh');
	 ...
	 public function check_status()
	 {
        ...
        $value  = $_POST['value'];
			   ...
			   $data_p_relation =
                ...
                $db_command = $this->Mdl_bank->db_command($data_p_relation,$id,'check_list');
    ...
    public function product_submit()
    {
        ...
        $product_code = $this->Mdl_bank->_get_product_code_generator();
       ...
       // exit;
        if($post['id'] > 0)
        {
            ...
            $data_p_relation =
                ...
                (
                    ...
                    'is_new'                   => $post['product_type'],
                    'product_name'             => $post['product_name'],
                    'product_description'      => $post['product_des'],
                    'product_featured'         => $post['product_fea'],
                    ...
                    'product_quantity'         => $post['qty'],
                    'product_image'            => $cat_image,
                    'featured_image'           => $featured_image,
                    'user_type'                => $user_type,
                    'is_enable'                => $is_enable
                );
            $db_command = $this->Mdl_bank->db_command($data_p_relation,$post['id'],'product_list');
            $this->db->trans_complete();
            if($db_command)
            {
                if($post['id'] > 0)
                ...
                redirect(base_url().'Bank/product_list','refresh');
            }
            else
            {
                $this->session->set_flashdata('Error', 'your data has not been saved');
        ...
        else
        {
           ...
           // exit;
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation,null,'product_list');
            $this->db->trans_complete();


            if($db_command)
            {
                //Send Email Admin Bank ADD Seller
                if($this->session->userdata('user_type') == 'seller')
                {
                    ...
                    $msg1 = "Dear Admin Seller Bank Add at Online Shopping";
                    $msg1 .= "<br/><a href='".$get_link1."' >CLICK HERE</a> to login your account and view product detail to active product in website.<br/>Thank you";
                    $this->email->from($from_email1, 'Online Shopping');
                    $this->email->to($to_email1);
                    $this->email->subject('Seller Bank Add at Online Shopping');
                        ...
                        <div class="table-responsive ">
                            <table id="product-table" class="table table-striped table-bordered table-hover">
                                ...
                                foreach($result as $res):
                                    ...
                                    $category_name = $this->Mdl_bank->get_relation_pax('category_list','category_name','id',$res->category_id);
                                    if($res->user_type == "seller") {
                                        $seller_name = $this->Mdl_bank->get_relation_pax('seller_list', 'company', 'id', $res->created_by);
                                    }else
                                    {
                                        $seller_name = "--";
                                    }
                                    $subcategory_name = $this->Mdl_bank->get_relation_pax('category_list','category_name','id',$res->subcategory_id);
                                   // $currency_name = $this->Mdl_Product->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="green" target="_blank" href="<?php echo base_url('Bank/view_image/'.$res->id.''); ?>">
                                        ...
                                        <td>
                                            <label class=" inline">
                                                <input  ref="<?php echo $res->id; ?>" reff="<?php if($res->user_type == 'seller'){ echo $res->created_by; } ?>"  <?php if($res->is_enable == 0){ echo 'value="0" ';}else{echo 'value="1" checked="checked"';}  ?>    type="checkbox" class="ace ace-switch ace-switch-5 <?php if($this->session->userdata('user_type') != 'seller'){ echo "product_status"; } ?>" />
                                                <span class="lbl"></span>
                                            </label>
                                        </td>
                                        ...
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                ...
                                                <a class="green"  href="<?php echo base_url('Bank/product_action/'.$res->id.''); ?>">
                                            ...
                                            <div class="visible-xs visible-sm hidden-md hidden-lg">
                                                <div class="inline position-relative">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
                                                        ...
                                                        <li>
                                                            <a href="<?php echo base_url('Bank/product_action/'.$res->id.''); ?>" class="tooltip-success" data-rel="tooltip" title="Edit">
                                                                <span class="green">
            ...
            jQuery(function($)
            {
                ...
                $(".product_status").change(function(){
                    ...
                    if(ref != null)
                    {
                        ...
                        var request = $.ajax({
                            url: "<?php echo base_url('Bank/change_product_status'); ?>",
                            type: "POST",
                            data: json,
                            dataType: "json",
                           // dataType: "html",
                            ...
                            success : function(_return)
                            {
                                ...
                                if(_return['_return'] > 0)
                                {
                                    ...
                                    $.gritter.add({
                                        ...
                                        text: 'Bank Status Changed Successfully.',
  ...
  /*              //Discount Status Change
                $(".discount_status").change(function(){
                    ...
                    if(ref != null)
                    {
                        ...
                        json['status']   = status_new;
                        var request = $.ajax({
                            url: "<?php //echo base_url('Bank/change_discount_status'); ?>",
                            type: "POST",
                            data: json,
                            dataType: "json",
                                ...
                                foreach($result as $res):
                                   ...
                                   // $currency_name = $this->Mdl_Product->get_relation_pax('currency_list','code','id',$fc->currency_id);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a class="green btn" target="_blank" href="<?php echo base_url('Bank/add_order/'.$res->id.''); ?>">
                                               Buy Now
                                            </a>
                                        </td>
...
*/
   ...
   //Add New Order
	...
	/* 	$get_order = $this->Mdl_bank->_get_order_code_generator();
    ...
    {	
		...
		exit; */
				...
				if($paid_price < $order_remaining_amount1)
				{
					...
					$db_command = $this->Mdl_bank->db_command($data_p_relation2111,null,'payment_list');
            ...
            if($db_command)
            {
				...
				//$data['content']    = 'Bank/view_order_slip';
				$this->load->view('Bank/view_order_slip',$data);
            }
            else
            {
                $this->session->set_flashdata('Error', 'your payment has not been saved');
                redirect(base_url().'Bank/add_payment','refresh');
    ...
    public function order_submit()
    {
		...
		//$this->db->trans_start(); // Query will be rolled back
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation1,null,'user_list');
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation1,$post['order_user_id'],'user_list');
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation_order,null,'order_list');
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation_cart,null,'cart_list');
            //$db_command = $this->db->insert('order_list', $data_p_relation_order);
			$last_cart_id = $this->db->insert_id();
				 ...
				 $get_product_quantity = $this->Mdl_bank->get_relation_pax('product_list','product_quantity','id',$pid);
            ...
            $db_command = $this->Mdl_bank->db_command($data_p_relation2,null,'payment_list');
            ...
            else
            {
                ...
                redirect(base_url().'Bank/add_order/'.$post['product_id'],'refresh');
    ...
    public function view_image()
    {
        ...
        $data['content']  = 'Bank/product_view_image';
        $this->load->view('Template/template',$data);
    ...
    public function distribution()
    {
        ...
        $data['content']  = 'Bank/distribution';
        $this->load->view('Template/template',$data);   
    }
    ...
    { 
        ...
        $data['content']   = 'Bank/customer_action';
        $this->load->view('Template/template',$data);
    }
    ...
    public function customer_command()
    {
        ...
        if($db_command)
        {
            if($post['customer_id'] != null)
            {
                ...
                $this->session->set_flashdata('update', 'your data successfully Updated');  
                redirect(base_url()."Bank/customer",'refresh'); 
            }
            else
            {
                ...
                $this->session->set_flashdata('saved', 'your data successfully Saved'); 
                redirect(base_url()."Bank/success/$get_code/$code_gen/$get_code",'refresh'); 
            }
    ...
    {
        //$data['result_set'] = $this->Mdl_Product->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Bank/payment_receiving';
        $this->load->view('Template/template',$data);   
    }
    ...
    public function payment_receiving_search()
    {
        ...
        <div class="clearfix Bank-actions" style="text-align: center;">
    ...
    public function payment_submit()
    {
        ...
        redirect(base_url()."Bank/payment_receiving",'refresh'); 
    }
    ...
    public function actionSms($number,$msg)
    {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER,     array('Content-Type: application/x-www-Bank-urlencoded'));
    ...
    {
        //$data['result_set'] = $this->Mdl_Product->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Bank/mailing_list';
        $this->load->view('Template/template',$data);   
    }
                                ...
                                <tbody>
                                    <?php foreach($filtered_customer as $fc): 
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $fc->noofcopy; ?> </td>
                                            <td>
                                                <a href="<?php echo base_url('Bank/print_card/'.$fc->inv_id.''); ?>" class="tooltip-error" data-rel="tooltip" title="View" target="_blank" >
                                                    <span class="purple">
    ...
    public function dispatch_submit()
    {
       ...
       // echo "<pre>";
        ...
        redirect(base_url()."Bank/dispatch_list",'refresh'); 
    }
    ...
    public function print_card()
    {
        ...
        if($id != null)
        {
            if($id  == 0)
            {
                ...
                redirect(base_url().'Bank/mailing_list','refresh');   
            }
            else
            {
                $this->db->where('id',$id);
    ...
    { 
        ...
        if(isset($_POST['save_inventory']))
        {
            ...
            if($db_command)
            {
                ...
                redirect(base_url().'Bank/inventory_receiving','refresh');  
            } 
        }
        else 
        {
    ...
    {
        //$data['result_set'] = $this->Mdl_Product->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Bank/dispatch';
        $this->load->view('Template/template',$data);   
    }
	
	//Recent SMS List Bank
	public function resend_sms()
    {
        //$data['result_set'] = $this->Mdl_Product->_this_controller_record_customer_distribution(); 
        $data['content']  = 'Bank/resend_sms';
        $this->load->view('Template/template',$data);   
    }
    ...
    public function recent_sms_submit()
    {
        ...
        redirect(base_url()."Bank/resend_sms",'refresh'); 
    }
    ...
    public function get_subcategory_byid()
    {
        ...
        $data['result_set'] = $this->Mdl_bank->sub_category_list($search_cat_id);
    ...
    public function get_product_byid()
    {
        ...
        $_return['_return'] = $this->Mdl_bank->_get_single_product_byCategory($search_cat_id,$scat_id);
        $return             = json_encode($_return);
        echo $return;
    ...
    public function change_bank_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_bank->_change_bank_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
    ...
    public function change_product_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_bank->_change_product_status($id,$status);
        if($_return['_return'] == 1)
        {
            //Active Account
            if($status == 0)
            {
                $seller_detail = $this->Mdl_bank->_this_seller_controller_record($seller_id);
                $from_email1 = $this->session->userdata('email');
                $to_email1 = $seller_detail[0]->email;
                 //Load email library
                $get_link1 = base_url()."admin";
                $username = $seller_detail[0]->username;
                $msg1 = "Dear ".$$username."<br/>Your Bank has been Active at Online Shopping";
                $msg1 .= "<br/><a href='".$get_link1."' >CLICK HERE</a> to login your account and view your products.<br/>Thank you";
                $this->email->from($from_email1, 'Online Shopping');
                $this->email->to($to_email1);
                $this->email->subject('Seller Bank Active at Online Shopping');
                $this->email->message($msg1);
                $this->email->send();
            ...
            else
            {
                ...
                $get_link1 = base_url()."admin";
                $username = $seller_detail[0]->username;
                $msg1 = "Dear ".$$username."<br/>Your Bank has been De-Active at Online Shopping";
                $msg1 .= "<br/>please contact site admin.<br/>Thank you";
                $this->email->from($from_email1, 'Online Shopping');
                $this->email->to($to_email1);
                $this->email->subject('Seller Bank DeActive at Online Shopping');
                $this->email->message($msg1);
                $this->email->send();
    ...
    public function change_discount_status()
    {
        ...
        $_return['_return'] =  $this->Mdl_bank->_change_discount_status($id,$status);
        $return             = json_encode($_return);
        echo $return;
