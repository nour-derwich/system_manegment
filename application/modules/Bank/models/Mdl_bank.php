...
class Mdl_bank extends CI_Model
{
	...
	//Get Bank List
    public function _get_bank_list($id)
    {
        if($id > 0)
        {
            $this->db->where('id',$id);
	   ...
	   //Get Check List
    public function _get_check_list($id)
    {
        if($id > 0)
        {
            $this->db->where('id',$id);
        }
		$this->db->where('check_type','seller');
        $query =  $this->db->get('check_list');
        return $query->result();
    }
	
	   //Get Check List
    public function _get_check_list1($id)
    {
        if($id > 0)
        {
            $this->db->where('id',$id);
        }
		$this->db->where('check_type','buyer');
        $query =  $this->db->get('check_list');
        return $query->result();
    }
 ...
 */
    ...
    //Change Bank Status
    public function _change_bank_status($id,$status)
    {
        if($id != null)
        {
            $data['is_enable'] = $status;
            $update =  $this->db_command($data,$id,'bank_list');
            if($update)
            {
                return 1;
            }else
